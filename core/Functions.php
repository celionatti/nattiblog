...
function get_image(mixed $file = '', string $type = 'post'): string
{
  ...
  if ($type == 'user') {
    return ROOT . "assets/img/user.webp";
  } else {
...
function remove_images_from_content($content, $folder = "uploads/featured/")
{
  ...
  if (is_array($matches) && count($matches) > 0) {
    ...
    foreach ($matches[0] as $match) {
      ...
      if (strstr($matches2[0], 'data:')) {
        ...
        $new_content = str_replace($parts[0] . "," . $parts[1], 'src="' . ROOT . $filename, $new_content);
